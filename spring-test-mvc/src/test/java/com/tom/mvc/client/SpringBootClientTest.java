...
import com.tom.mvc.controller.UserRestController;
import com.tom.mvc.entity.User;
import com.tom.mvc.service.UserService;
...
public class SpringBootClientTest extends AbstractClientTest {
    ...
    public void testFindById() throws Exception {
        ...
        assertThat(entity.getBody(), hasProperty("name", is("tom")));
    }
...
/*通过SpringApplication.run启动一个服务器，然后Config.xml是Spring的Java配置方式，
此处只加载了UserRestController及其依赖UserService，对于UserService可以通过如Mockito进行模拟/也可以注入真实的实现，Mockito请参考《单元测试系列之2：模拟利器Mockito》。可以通过EmbeddedServletContainerFactory子类指定使用哪个内嵌的web容器（目前支持：jetty/tomcat）。
这种方式的优点就是速度比内嵌Jetty容器速度快，但是还是不够快且还需要启动一个服务器（开一个端口），
因此Spring 3.2提供了模拟Server的方式进行测试。即服务器是通过Mock技术模拟的而不是真的启动一个服务器。
*/
    @Configuration
