...
import com.photograph.common.springmvc.DateConvertEditor;
import com.photograph.pojo.*;
import com.photograph.service.BaseUserRoleService;
import com.photograph.service.BaseUserService;
import com.photograph.web.interseptor.WebConstants;
...
public class UserController {
	...
	public Object all(ExtPager pager, @RequestParam(required = false, defaultValue = "") String realName) {
		...
		List<BaseUser> list = this.baseUserService.selectByExample(criteria);
		int total = this.baseUserService.countByExample(criteria);
	...
	public Object changePassword(@RequestParam String oldPassword, @RequestParam String newPassword, @RequestParam String comparePassword,
		...
		try {
			...
			String result = this.baseUserService.updateUserPassword(criteria);
			if ("01".equals(result)) {
	...
	public Object resetPassword(@PathVariable String userId) {
		try {
			...
			Criteria criteria = new Criteria();
			criteria.put("userId", userId);
			String result = this.baseUserService.resetPwdByPrimaryKey(criteria);
			if ("01".equals(result)) {
	...
	public Object delete(@PathVariable String userId, HttpSession session) {
		try {
			...
			Criteria criteria = new Criteria();
			criteria.put("userId", userId);
			String result = this.baseUserService.deleteByPrimaryKey(criteria);
			if ("01".equals(result)) {
	...
	public Object validateAccount(@RequestParam(value = "account", required = false, defaultValue = "") String account,
		...
		try {
			...
			String result = this.baseUserService.validateAccount(criteria);
			if ("01".equals(result)) {
	...
	public Object save(BaseUser user, @RequestParam Collection<String> roleIds) {
		try {
			...
			criteria.put("user", user);
			String result = this.baseUserService.saveUser(criteria);
			if ("01".equals(result)) {
	...
	public Object saveMyinfo(BaseUser user) {
		try {
			...
			String result = this.baseUserService.updateByPrimaryKeySelective(user);
			if ("01".equals(result)) {
