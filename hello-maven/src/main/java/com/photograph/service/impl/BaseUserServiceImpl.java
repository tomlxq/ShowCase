...
import com.photograph.common.utils.EncryptUtil;
import com.photograph.dao.BaseUserMapper;
...
public class BaseUserServiceImpl implements BaseUserService {
	@Autowired
	private BaseUserMapper baseUsersMapper;
	@Autowired
	...
	public String selectByBaseUser(Criteria criteria) {
		// 条件查询
		List<BaseUser> list = this.baseUsersMapper.selectByExample(criteria);
	...
	public String resetPwdByPrimaryKey(Criteria example) {
		String userId = example.getString("userId");
		BaseUser oldUser = this.baseUsersMapper.selectByPrimaryKey(userId);
		if (oldUser == null) {
			return "没有找到该用户！";
		}
	...
	public String updatePassword(Criteria example) {
		BaseUser user = this.baseUsersMapper.selectByPrimaryKey(example.getString("userId"));
		if (user == null) {
			return "00";
		}
		BaseUser updateUser = new BaseUser();
	...
	public String findPassword(Criteria example) throws Exception {
		...
		List<BaseUser> list = this.baseUsersMapper.selectByExample(example);
		if (null == list || list.size() != 1) {
